...
class Model(Predict):
    ...
    def estimate_forecast(
        ...
        * Если ``max_rows`` не равен ``None`` и данных больше, чем
          ``max_rows``, автоматически берутся последние ``max_rows`` строк.
          Информация об усечении попадёт в ``ForecastResult.preprocessing`` и
        ...
        * ``rolling_sample_pct`` управляет прореживанием стартов rolling.
          Например, ``50`` означает «брать каждый второй старт». Значение
          ``100`` рассчитывает веер на всех доступных тактах.
        ...
        **Как использовать результат**
        ...
        rolling_sample_pct : float, по умолчанию ``100.0``
        ...
        max_rows : int или None, по умолчанию ``10_000``
